...
class Render3D {
    ...
    createTip(n, options) {
        let scope = this;
        //console.log(this)
        let limb = this.createLimb(this.robot.legs[n].sizes.tip.width, this.robot.legs[n].sizes.tip.height, this.robot.legs[n].sizes.tip.length, this.robot.legs[n].sizes.tip.offset);
    ...
    createLeg(n, options) {
        ...
        const mirror = this.robot.legs[n].mirror;
        ...
        let limb = this.createLimb(this.robot.legs[n].sizes.upper.width, this.robot.legs[n].sizes.upper.height, this.robot.legs[n].sizes.upper.length, this.robot.legs[n].sizes.upper.offset);
        ...
        upperGroup = this.moveMeshRotCenter(upperGroup, 0, 0, this.robot.legs[n].sizes.upper.length - ServoData.servo.ch)
        ...
        shoulder.rotate = function(v) {
            let degAngle = v + scope.robot.legs[n].angle + 90;
            if (scope.robot.legs[n].mirror) {
    ...
    createRobot() {
        ...
        // Create the body
        let body = this.createBody(this.robot.options);
        ...
        for (let i=0;i<this.robot.legs.length;i++) {
            // Create the leg
            let leg = this.createLeg(i, {});
            let angle = this.robot.legs[i].angle; //(360/gait.options.leg.count)*i + gait.body.angle;
            leg.angle = angle;
            leg.mesh.position.x = this.robot.legs[i].anchor.x;
            leg.mesh.position.z = this.robot.legs[i].anchor.y;
            ...
            let footPosition = this.create3DCircle({
                x: this.robot.legs[i].tip.x,
                y: 0,
                z: this.robot.legs[i].tip.y
